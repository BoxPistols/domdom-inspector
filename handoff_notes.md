...
> すべて **2026-07-07 時点の、ある一セッションの観測**にすぎない。project memory と同じで、当時の
...
## 残っている道
- 将来 Phase: FR-13(iframe 対応)、FR-14(テーマ辞書取得 = Phase 3 の中核かつ最大の技術リスク。ROADMAP は PoC 先行を推奨)、Phase 4/5。
- リファクタ続き: B-4(overlay サーフェス分割)、C(モード3クラス共通化 — ただし 4 点配線に触れる)、D(popup 注入抽出 — gesture 順序リスク)。
- CWS: 掲載文はもう現行機能と整合している。残るは実機スクショ・ポリシー公開 URL・$5 登録という人間の作業だけ。
