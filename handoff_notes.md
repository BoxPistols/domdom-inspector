...
- **v1 のスコープ。** ホバー計測 + MUI テーマ自動検出によるトークン照合 +
  右クリック/エディタ起動 + **トークンカバレッジ side panel (#10 完了で復帰)**。
  AI 監査 / 表示設定 / トークン貼り付けは配線から外して issue 化 (#11 #12 #13)。
  実装は温存 = 到達不能
- **第三者への送信はゼロ。** 発行する要求は 2 種類 (利用者自身のローカル dev サーバへの
  エディタ起動依頼 / source map 取得)、どちらも `src/openInEditor.ts` に集約
  (v0.4.23 / v0.4.33)。この事実に
