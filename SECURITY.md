...
- **第三者への送信経路がゼロ。** 拡張が発行するネットワーク要求は**2 種類**で、
  どちらも**利用者自身のローカル開発サーバ**宛て (送信 API は `src/openInEditor.ts` の
  1 ファイルに集約):
  (1) 「**このファイルをエディタで開いて**」と頼む要求 — 送るのは**そのページ自身が
  生成したソースパスと行番号**だけ
  (2) バンドル後の位置を元ソースへ戻すための **source map の取得** (React 19 が
  `_debugSource` を削除したため、位置は必ずバンドル座標で来る) — 取得先は**ページ自身が
  配信している URL** だけ
  どちらも宛先はページ自身のオリジンで、しかも `looksLocalDev` が真のとき
  (localhost / 127.0.0.1 / `*.local` / `*.test` / プライベート IP) **に限る**。
  それ以外のサイトでは 1 バイトも出さない。DOM・テキスト・入力値・利用者データは
  一切含まない。第三者のサーバへは何も送らない。
  - **正確な範囲**: 「利用者自身のローカル開発サーバ」とは実装上「**今見ているページ
    自身のオリジン** かつ `looksLocalDev` が真」のこと。`*.local` やプライベート IP には
    LAN 上の他者のサービスも含まれ得るが、その場合も要求が飛ぶのは**そのページを配信
    しているホスト自身** (利用者が意図的に開いてエディタ起動を操作したページ) だけで、
    内容はそのページ自身が生成したソースパス / 配信 URL に限られる。拡張が選んだ
    第三者ホストへ送る経路は存在しない。
...
| `sidePanel` | 計測結果を対象ページと**並べて**表示するため (カバレッジのパネル) | **ページへのアクセス権限は増えない**。公式 permissions-list で警告文を持たない権限。パネルは popup の「カバレッジのパネルを開く」からのみ開く |
