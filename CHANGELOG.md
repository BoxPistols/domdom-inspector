...
という問いへの回答。**デバッグ用ではなかったが、要らなくできた。**

まず実測: `REACT_EDITOR=antigravity-ide` では Next はファイル名だけを渡し、**行番号が落ちる**
(Next も `launch-editor` と同じ名前判定を持つ)。よって dev サーバ経由を使う限り shim は必要。

**しかし dev サーバ経由そのものが不要になっていた。** この経路は「ブラウザは絶対パスを
知り得ない」ことへの対策だったが、v0.4.33 で source map から**元ファイルの絶対パス**が
得られるようになり、前提が消えていた。

- **絶対パスが手元にあるときはスキームで直接開く**。dev サーバへは何も出さない。
  利用者の設定はゼロ (popup でエディタを選ぶだけ)
- **相対パスしか無いときだけ** dev サーバ経由へ落ちる (source map が無い構成 /
  React 18 以前)。設定が要るのはそのときだけ
- 配信パス (`/src/App.tsx`) を「絶対パス」と誤認しない (`isAbsoluteLocalPath`)

README (ja/en) を「設定は原則不要」に書き換えた。
直接開く分岐を外すとテストが赤になることを確認済み。

---

## 0.4.39 (2026-08-17) — React の中を避けるだけでなく、正しい場所へ落とす

v0.4.38 は「React の内部を開かない」までで止まっていた。**避けた先が無い**ので、
共有スタックに当たった要素では何も開かなくなる。欲しいのは
`components/input/Dropzone.tsx:178` のような**利用者のコードの位置**。

候補の順序を「**要素自身 → owner チェーン**」にした:

- 要素自身の Owner Stack が最も正確 (その JSX が書かれた行そのもの)
- React が実捕捉を打ち切って共有スタックになっていたら、owner チェーン
  (そのコンポーネントが書かれた場所) へ落とす。**1 段浅いが利用者のコードではある** —
  React の実装を開くよりはるかに良い
- 同じ位置は重複させず、上限 12 件

落とし先を消すとテストが赤になることを確認済み。
...
トーストは画面下中央に固定で出るので、要素からボタンまでポインタを運ぶ距離がある。
そこへ 9〜14 秒の自動消去を掛けていた。**操作を求めておいて時間で取り上げる**のと同じで、
猶予を何秒に伸ばしても「間に合わないことがある」は消えない。

- **操作可能トーストは自動で消さない**。閉じる手段を 2 つ置く: 操作ボタンを押す / ✕ を押す
- Esc でも閉じる (中央ハンドラの先頭。ハイライトやモードより先に消費する)
- 操作を押したら閉じる (押した後も居座らない)
- 本文は折り返して全部見せる (パスは長い)。ボタンは縮めない
- **通常のトーストは従来どおり自動で消える** (常時居座らせない)

自動消去に戻すとテストが赤になることを確認済み。
