...
**§2 の中核 (「素キー送信を全廃し、状態を推測して安全性を担保しない」) は生きている。**
今回導入したのは*表示*の条件分岐だけで、安全性は front 再照合というコード側の手当てで
取っている。展開状態は拡張のモードではなく **Touch Bar ローカルの状態**であり、
ズレる前提で「Chrome を離れたら畳む」ことで回収する (詳細は README)。
