...
> **追記 (2026-08-03): 一部の結論は後の決定で上書きされた。**
> 本書は「単一目的を 1 段上げてツリー/レンダー可視化を含める」を推奨しているが、
> その後の実機フィードバック (本番ビルドのツリーが `0e` / `Anonymous` で判読不能) を受けて
> **v1 では両機能を配線から外す**決定をした。単一目的も「計測 + トークン照合」に絞ってある。
> 経緯は `docs/ROADMAP.md` の「v1 スコープ決定 (2026-08-03)」を参照。
> 本書の他の結論 (観点別の棚卸し・競合調査・ストア審査リスク) はそのまま有効。
